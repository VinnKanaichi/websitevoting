    ...
    <main className="min-h-screen flex flex-col items-center justify-center gap-6 p-4">
      ...
      {!hasVoted && !isAdmin ? (
        <div className="flex flex-col gap-4 w-full max-w-md">
          <button
            onClick={() => handleVote('furina')}
            className="bg-indigo-600 p-4 rounded-xl text-white font-semibold"
          >
            BOT YANG SEKARANG FURINA
          </button>
          <button
            onClick={() => handleVote('multifungsi')}
            className="bg-purple-600 p-4 rounded-xl text-white font-semibold"
          >
            BOT MULTIFUNGSI
          </button>
        </div>
          ...
          <div>
            ...
            <div className="w-full bg-gray-700 rounded-full h-4">
              <div
                ...
                style={{ width: `${percent.multifungsi}%` }}
              />
            </div>
          </div>
      ...
      {isAdmin && (
        <div className="mt-6 text-center">
          <h2 className="font-bold mb-2">🎛️ Admin Panel</h2>
          <button
            onClick={() => handleVote('furina')}
            className="bg-green-600 px-4 py-2 rounded-xl text-white m-1"
          >
            +1 FURINA
          </button>
          <button
            onClick={() => handleVote('multifungsi')}
            className="bg-yellow-600 px-4 py-2 rounded-xl text-white m-1"
          >
            +1 MULTIFUNGSI
          </button>
        </div>
